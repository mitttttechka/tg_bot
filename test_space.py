...
def view_current_sort(track_state, person):
    track_state.change_state(ChangeState.view_current, person)
...
def await_new_sort(track_state, person, *add_text):
    track_state.change_state(ChangeState.await_new_sort, person)
    text = ''
    if len(add_text) > 0:
        text += add_text[0]
...
def update_learn_track(track_state, person, user_state):
    track_state.change_state(ChangeState.update_learn_track, person)
...
def await_current_sort(track_state, person, *add_text):
    track_state.change_state(ChangeState.await_current_sort, person)
...
def await_change_current(track_state, person, *add_text):
    track_state.change_state(ChangeState.await_change_current, person)
    mes = ''
    if len(add_text) > 0:
        mes += add_text[0]
...
def await_remove_task(track_state, person, *add_text):
    track_state.change_state(ChangeState.await_remove_task, person)
    mes = ''
    if len(add_text) > 0:
...
def update_add_task_middle(track_state, person, text):
    ...
    track_state.change_state(ChangeState.update_add_task_middle, person)
...
def await_set_position(track_state, person, *add_text):
    track_state.change_state(ChangeState.await_set_position, person)
    mes = ''
    if len(add_text) > 0:
        mes += add_text[0]
